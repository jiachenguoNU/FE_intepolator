# -*- coding: utf-8 -*-
"""
Finite Element Mesh and shape functions


adapted from Finite Element Digital Image Correlation method 
    ...
    def PlotContourDispl(self, U=None, n=None, s=1.0, stype='comp', newfig=True, **kwargs):
        ...
        **kwargs : TYPE
        ...
        if stype == 'mag':
            ...
            ux = ax1.tricontourf(n[:, 0], n[:, 1], triangles, V[self.conn[:, 0]], 20, alpha=alpha)
            #self.Plot(n=n, alpha=0.1)
            # plt.axis("off")
            # plt.title("x")
            fig1.colorbar(ux)
            ...
            uy = ax2.tricontourf(n[:, 0], n[:, 1], triangles, V[self.conn[:, 1]], 20, alpha=alpha)
            fig1.colorbar(uy)
            ax2.set_title('U_y')
            # self.Plot(n=n, alpha=0.1)
            # plt.axis("equal")
